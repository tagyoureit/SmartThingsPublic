...
def parse(String description) {
    ...
    if (msg.json.circuit != null) {
        ...
        if (debug) {
            log.debug("no Circuits in msg")
        }
    }
    if (msg.json.time != null) {
        parseTime(msg.json.time)
    } else {
        if (debug) {
            log.debug("no Time in msg")
        }
    }
    if (msg.json.schedule != null) {
        parseSchedule(msg.json.schedule)
    } else {
        if (debug) {
            log.debug("no Schedule in msg")
        }
    }
    if (msg.json.pump != null) {
        parsePump(msg.json.pump)
    } else {
        if (debug) {
            log.debug("no Pumps in msg")
        }
    }
    if (msg.json.valve != null) {
        parseValve(msg.json.valve)
    } else {
        if (debug) {
            log.debug("no Valve in msg")
        }
    }
    if (msg.json.chlorinator != null) {
        ...
        if (debug) {
            log.debug("no Chlor in msg")
        }
    }
    if (msg.json.intellichem != null) {
        parseIntellichem(msg.json.intellichem)
    } else {
        if (debug) {
            log.debug("no Chem in msg")
        }
    }
        ...
        retVal = httpGet(params) { response ->
            ...
            if (response.data?.Status == 0) { // Success
                for (answer in response.data?.Answer) {
                    // Loop through results looking for the first IP address returned otherwise it's redirects
                    //log.trace "Processing response: ${answer}"
                    if (isIPAddress(answer?.data)) {
                        log.trace "Hostname ${answer?.name} has IP Address ${answer?.data}"
                        return answer?.data
                        // We're done here (if there are more ignore it, we'll use the first IP address returned)
                    } else {
