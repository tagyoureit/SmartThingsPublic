...
void verifyDevices() {
    ...
    devices.each {
        ...
        sendHubCommand(new physicalgraph.device.HubAction("""GET ${
            it.value.ssdpPath
...
def addManualDevice() {
    if (controllerMac && controllerIP && controllerPort) {
        createOrUpdateDevice(controllerMac, controllerIP, controllerPort)
    }
}
